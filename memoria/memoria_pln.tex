...
En las últimas décadas se puede apreciar una tendencia creciente del uso de la inteligencia artificial en muchos de los productos y servicios que se consumen, en este contexto se aprecia el procesamiento de lenguaje natural como una de las ramas que ha tenido gran impacto en la interacción humano-computador y en el análisis automático de texto y audio, permitiendo nuevas generaciones de chatbots, resumen de información, análisis de sentimiento y detección de entidades entre muchas otras utilidades.\\
...
En 2006 y 2014 la organización i2b2 propone dos tareas para poder anonimizar de forma efectiva informes clínicos en inglés, posteriormente, en 2019 la iniciativa IberLEF 2019 lanza el reto de realizar esta tarea para textos clínicos en español bajo el nombre de MEDDOCAN (Medical Document Anonymization). En esta tarea se han involucrado algunas organizaciones como el gobierno de España a partir de su plan de impulso de tecnologías del lenguaje, el Centro Nacional de Investigaciones Oncológicas (CNIO) o el Centro Nacional de Supercomputación de Barcelona.\\
La motivación principal de esta tarea consiste en cumplir con la ley de protección de datos, dado que en la actualidad la información médica no puede ser compartida sin ser anonimizada apropiadamente, esto impide poder realizar muchas investigaciones en el campo del procesamiento de lenguaje natural en medicina debido a la falta de corpus disponible y a las implicaciones de trabajar con datos protegidos en caso de no anonimizar. La resolución exitosa de esta tarea permitirá automatizar este trabajo y abrir paso a futuros proyectos que trabajen con estos datos.

\section{Objetivo}
La finalidad de este proyecto es identificar los elementos necesarios para una anonimización de textos como la requerida en la tarea, para ello se usará el conjunto de datos proporcionados en MEDDOCAN y se acometerán las dos subtareas que se proponen:
\begin{itemize}
...
\end{itemize}
Para la evaluación de la calidad del algoritmo se usará la herramienta de evaluación de MEDDOCAN y se realizará un análisis de los resultados para posibles mejoras.\\
Adicionalmente, como objetivo secundario, se desarrollará una herramienta que permita la detección de cualquier tipo de entidad con la que se haya entrenado.

\section{Conceptos}
A lo largo de esta lectura se hará mención a algunos conceptos fundamentales para entender el funcionamiento de algunas partes del sistema. Estos se presentan a continuación.
...
En este caso se han decidido anotar las organizaciones y los países. Solo se indica la posición y etiquetas, aunque existe más información posible con este formato, en este estudio se va a usar únicamente la información de entidades nombradas.\\
El formato define que cada linea contiene una etiqueta y en cada linea se tiene en este orden un número de token (T\#), una tabulación, la etiqueta correspondiente, un espacio, el inicio y final del texto correspondiente separado por espacio, una tabulación y el texto al que corresponde. La extensión que se debe indicar para este tipo de ficheros es \textbf{.ann} y es un fichero de texto.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ESTADO DEL ARTE %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}
\item Entrenamiento: Se preparan los datos de entrenamiento de forma que se identifiquen todas las etiquetas de cada texto para, partiendo de un modelo preentrenado, ajustase al contexto del problema entrenando con estos datos.
...
\item Validación: Si se desea valorar la efectividad del modelo se puede contrastar las predicciones del modelo contra anotaciones manuales.
\end{itemize}
...
Un diagrama general del sistema planteado es el siguiente:
\begin{figure}[H]
\centering
...
\item Models: Serán los encargado de realizar las predicciones para resolver la tarea.
...
Quiero pueda entrenar un modelo y guardarlo.\\
...
HU11: 5PH\\
Como usuario de la herramienta.\\
Quiero pueda usar un modelo guardado para generar predicciones.\\
...
El sistema ha sido implementado en python, para la resolución de las tareas de MEDDOCAN se han usado 4 notebooks de la herramienta Jupyter que permite ejecutar código en una interfaz web. Estos se corresponden a los módulos que se describen en \ref{esquema}.\\
Adicionalmente se desarrolló una herramienta que permite entrenar modelos de forma automática a partir de textos anotados, aprendiendo las etiquetas incluidas en esos textos.
...
En primer lugar se carga el modelo de Spacy en español descartando las partes del pipeline que no se corresponden al NER(Named Entity Recognition) dado que será la única parte que se pretende reentrenar.\\
...
Los valores parámetros que se van a modificar para crear modelos diferentes son el número de iteraciones con las que se entrena y el dropout de la red neuronal.\\
...
En primer lugar se carga el modelo que se indica en la entrada. Para cada texto se lee completo y se sustituyen los saltos de linea por espacios de la misma forma que los usó el modelo al entrenar, después de eso se realiza una predicción del texto que consiste en un conjunto de entidades de las que se extrae un diccionario con las claves \textit{inicio}, \textit{final}, \textit{etiqueta} y \textit{texto}.\\
\subsubsection{Generar la anotación}
Una vez se tiene el conjunto de diccionarios con las etiquetas que ha encontrado el modelo, se copia el texto original al directorio de destino y se genera la anotación en formato brat, para ello se usa un diccionario por linea para generar el formato mencionado en \ref{brat} incrementando el número del token de la forma correspondiente, el fichero de la anotación tiene el mismo nombre que el fichero original pero con extensión ann.
...
En primer lugar se lee el texto como en los apartados anteriores sustituyendo los saltos de linea por espacios.\\
...
En la carpeta de referencia se debe ubicar una carpeta llamada brat que contenga los textos con la anotación válida.\\
 ...
 Para realizar esta tarea de forma automática para todos los modelos se copiará la anotación correspondiente al modelo a las carpetas correspondientes en le estructura de directorios, se ejecutará el script y se cambia el nombre de la salida para que no lo sobreescriba.
...
En el siguiente esquema se puede ver de forma general la estructura de la herramienta.\\
...
\cleardoublepage % empezamos en página impar
\chapter{Resultados}
En primer lugar se entrenó el modelo de Spacy usando los datos de entrenamiento, de todos los parámetros probados se recopilaron las puntuaciones proporcionadas por la herramienta de MEDDOCAN.\\
...
\section{Validación}
En primer lugar se analizó la puntuación de los modelos en el conjunto de validación, el resultado de la primera tarea se puede ver en la tabla \ref{tab:tabla-dev} donde se muestran los 15 con mejor puntuación F1, de la misma forma en la tabla \ref{tab:tabla-dev-2} se muestran los resultados de la segunda tarea según en criterio estricto.\\
...
Tras este análisis se procede a comprobar las etiquetas que no reconoce el modelo de referencia en busca de posibles patrones en los errores, dando lugar a la tabla \ref{tab:tabla-miss}

\begin{table}[H]
\centering
\begin{tabular}{|l|l|}
\hline
\multicolumn{1}{|c|}{\textbf{Etiqueta}} & \multicolumn{1}{c|}{\textbf{Fallos}} \\ \hline
...
Con esta información se realiza un estudio de las posibles expresiones regulares para mejorar el sistema, obteniendo tras ello los resultados de las tablas \ref{tab:tabla-dev-pos} y \ref{tab:tabla-dev-2-pos} correspondientes a la subtarea 1 y 2 respectivamente una vez se ha realizado el postproceso.
