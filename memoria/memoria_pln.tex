...
En medicina es difícil realizar investigación con informes médicos dado que existe mucha información sensible que no puede cederse sin pasar por un proceso previo que la anonimice, esto crea una barrera que impide el tratamiento automático de informes médicos.
Este trabajo se ha centrado en resolver la tarea MEDDOCAN, esta consiste en identificar entidades nombradas que requieren ser anonimizadas en informes clínicos en castellano. El sistema planteado hace uso de un modelo preentrenado que se adapta al contexto entrenando con los datos de la tarea. Tras diversas iteraciones analizando los resultados del sistema, se incluye un postproceso sobre los resultados para mejorarlos.\\
...
En 2006 y 2014 la organización i2b2 propone dos tareas \cite{i2b2} para poder anonimizar de forma efectiva informes clínicos en inglés, posteriormente, en 2019 la iniciativa IberLEF 2019 lanza el reto de realizar esta tarea para textos clínicos en español bajo el nombre de MEDDOCAN (Medical Document Anonymization) \cite{MEDDOCAN}. En esta tarea se han involucrado algunas organizaciones como el Gobierno de España a partir de su Plan de Impulso de Tecnologías del Lenguaje \cite{PlanTL}, el Centro Nacional de Investigaciones Oncológicas (CNIO) \cite{CNIO} o el Centro Nacional de Supercomputación de Barcelona \cite{CSB}.\\
...
En la Figura \ref{fig:gold} se muestra un ejemplo de anotación de documento para la tarea MEDDOCAN
\begin{figure}[H]
\centering
\includegraphics[scale=0.60]{img/meddocan-tags-1024x146.png}\\
\caption{Ejemplo de anotación de referencia de un documento.}
\label{fig:gold}
\end{figure}

Un ejemplo de anotación de salida de un sistema sería el mostrado en la Figura \ref{fig:s1}, y sería correspondiente a la salida de la subtarea 1. Si se compara esta salida con la anotación de la Figura \ref{fig:gold}, que sería la anotación de referencia, se puede ver cómo todo lo que ha detectado está bien, tanto por el tipo de entidad identificado como por sus posiciones. Sin embargo, no ha encontrado todas las posibles entidades que podría haber identificado.
...
Para la subtarea 2 el ejemplo propuesto es el de la Figura \ref{fig:s2}, donde se tiene una puntuación perfecta debido a que ha encontrado todas las etiquetas con su localización correcta, no se tiene en cuenta el tipo de entidad por lo que podría indicarse cualquiera sin alterar la puntuación.

\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{img/meddocan-ex3-1.png}\\
\caption{Anotación de ejemplo de la subtarea 2}
\label{fig:s2}
\end{figure}
...
URL\_WEB                                \\ \hline
\end{tabular}
\caption{\label{tab:tags} Categorías posibles a anonimizar}
\end{table}


\section{Conceptos}
A lo largo de esta memoria se hará mención a algunos conceptos fundamentales para entender el funcionamiento de algunas partes del sistema. A continuación se presentan estos conceptos. 
...
En el contexto del PLN se denomina embedding a una técnica de representación del lenguaje que permite transformar tokens en vectores numéricos, esto será útil para utilizar otras herramientas como redes neuronales que no sean capaces de tratar texto de forma directa.\\
...
Esta memoria se encuentra dividida en 7 capítulos, el contenido de los 6 restantes se presenta a continuación::
\begin{itemize}
\item Capítulo 2: Se muestra el estado del arte donde se expone los sistemas y métodos que han usado los mejores participantes de la tarea MEDDOCAN para abarcar el problema
\item Capítulo 3: Descripción de las diferentes partes del sistema propuesto que se ha implementado.
\item Capítulo 4: Metodologías de software y tecnologías empleadas en el proceso.
...
\cleardoublepage
\chapter{Mejores propuestas de la tarea MEDDOCAN}
En este capítulo se recogen algunas de las propuestas de los participantes con mejores resultados en la tarea \cite{fadi}\cite{lukas}\cite{nperez}.

\section{Red Neuronal Bi-LSTM con CRF}
Este tipo de propuestas consta de dos elementos que se detallan a continuación, por un lado una red neuronal que recibe los tokens vectorizados a traves de un embedding y un modelo CRF que usa la salida de la red para detectar las etiquetas correspondientes.
\subsection{Red Neuronal Recurrente}
En este tipo de red neuronal la salida de la red se usa a su vez como entrada creando una dependencia temporal que avanza a medida que se entrena. Se puede ver de forma simplificada la estructura de un único nodo en la Figura \ref{fig:rnn-1}.

\begin{figure}[H]
\centering
\includegraphics[scale=1.0]{img/RNN-1.png}\\
\caption{Red Neuronal Recurrente}
\label{fig:rnn-1}
\end{figure}

Esta estructura se puede representar con un eje temporal que ayuda a interpretar las dependencias entre las diferentes entradas, un ejemplo de esta representación se encuentra en la Figura \ref{fig:rnn-2}.

\begin{figure}[H]
\centering
\includegraphics[scale=1.0]{img/RNN-2.png}\\
\caption{Red Neuronal Recurrente Extendida}
\label{fig:rnn-2}
\end{figure}

\subsection{Red Neuronal Bi-LSTM}
Una red neuronal Bi-LSTM (Long Short Term Memory) presenta una arquitectura que puede interpretarse como la combinación de dos redes neuronales recurrentes independientes, donde una envía la salida de cada entrada hacia adelante y la otra hacia atrás.\\

Las redes LSTM tienen una particularidad y es que, a diferencia de una red neuronal recurrente donde el peso del primer elemento se ve reducido en cada nuevo elemento, en este tipo de redes se incorpora un elemento de entrada adicional llamado estado, este elemento es actualizado en cada iteración dejando la información relevante aunque provenga de elementos muy lejanos.


Esta arquitectura puede verse en la Figura \ref{fig:Bi-LSTM}.

\begin{figure}[H]
\centering
\includegraphics[scale=0.8]{img/LSTM.png}\\
\caption{Red Neuronal Bi-LSTM}
\label{fig:Bi-LSTM}
\end{figure}

\subsection{Campo Aleatorio Condicional}
Un campo aleatorio condicional (o CRF por sus siglas en inglés) es un modelo estocástico para reconocer patrones que se utiliza en este caso para asignar a cada token la etiqueta correspondiente en función de la salida de la red.\\

Este modelo usa el contexto del token (salida de los tokens vecinos) para predecir la etiqueta más probable a cada token, este problema se resuelve con el algoritmo de Viterbi. Hay que observar que este modelo asigna una etiqueta por token, por lo que luego hay que reconstruir la secuencia completa de tokens que corresponden a una entidad, para ello además del tipo de entidad también se indica la posición del token en la entidad siguiendo el esquema BIOES (Begin, Inside, Outside, End, Single).\\

Existe la etiqueta vacía que se denota con la letra O e indica que no pertenece a ninguna entidad el token correspondiente.
\subsection{Esquema del sistema}
Uniendo los diferentes conceptos mostrados anteriormente podemos ver una estructura de este sistema en la Figura \ref{fig:bi-lstm-crf}.
\begin{figure}[H]
\centering
\includegraphics[scale=1.0]{img/bi-lstm.png}\\
\caption{Arquitectura Bi-LSTM con CRF}
\label{fig:bi-lstm-crf}
\end{figure}

\section{Blooming Embedding con Residual CNN}
Esta aproximación será la usada en el sistema planteado ya que es la que utiliza la herramienta SpaCy. Consiste en usar un tipo de representación de tokens basado en embeddings junto con una red neuronal convolucional (CNN) residual. Este sistema también es utilizado por el participante \cite{nperez}.
\subsection{Blooming Embedding}
Habitualmente las tecnicas de embedding usan un vocabulario de $N$ palabras conocidas a partir de la cual se crea una matriz de tamaño $N\times L$ donde $L$ será la longitud del vector que se quiere asignar a cada palabra, esto genera el problema de que el tamaño del embedding aumenta con las palabras del vocabulario, por otro lado las palabras que no se encuentren en el vocabulario deben ser asignadas tambien a algún vector para poder representar todos los textos posibles, se suele asignar un conjunto de filas de la matriz de embedding a palabras desconocidas para solventar este problema, no obstante genera una nueva problemática ya que palabras muy diferentes pueden ser asignadas a la misma representación por lo que puede llevar a error.\\

Blooming Embedding es un tipo de embedding donde no se tiene un vocabulario de palabras conocidas con una representación muy precisa a cambio de tener toda la matriz para asignar cualquier palabra, esto se hace aplicando una función hash para crear categorías donde cada palabra va a una representación mas o menos aproximada de su significado. Esta representación permite obtener representaciones para una gran cantidad de palabras a la vez que mantiene un tamaño reducido y elimina parcialmente el problema de las palabras desconocidas. Para terminar de solventar el problema de palabras muy variadas que van a la misma representación se usa una segunda representación basada en otra función hash diferente y al combinar ambas representaciones se obtiene una representación que con alta probabilidad es única para cada palabra con todas las ventajas descritas anteriormente.

\subsection{Residual Convolutional Neural Network}
Una red neuronal convolucional es un tipo de red neuronal que presenta una arquitectura basada en diferentes tipos de capas, la entrada de estas redes es un tensor, que en el caso que corresponde al sistema serán diferentes subconjuntos de tokens representados a traves del embedding, los diferentes tipos de capas son:
\begin{itemize}
\item Convolución: Este tipo de capas tiene como entrada de cada nodo un tensor y permite extraer características dando como resultado una matriz de características para cada nodo, el tamaño del tensor de entrada y la salida no tienen por que coincidir.
\item Muestreo: En estas capas se reduce la dimensión de su entrada aplicando algún tipo de función como el máximo o la media.
\item Completamente Conectada: Es una capa donde cada salida anterior se conecta a cada entrada de cada nodo de esta.
\item Clasificación: Esta sería una capa de salida para la red neuronal, la salida está determinada por vector de peso e influencia que se irá ajustando a medida que entrene.
\end{itemize}

Un ejemplo que ilustra las diferentes capas se puede ver en la Figura \ref{fig:CNN}.

\begin{figure}[H]
\centering
\includegraphics[scale=0.35]{img/CNN.png}\\
\caption{Capas Red Neuronal Recurrente}
\label{fig:CNN}
\end{figure}

El concepto residual se puede aplicar a diferentes tipos de redes neuronales, implica que la salida de una capa pueda dirigirse no solo a la siguiente sino a otra capa saltándose varias de ellas.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DESARROLLO %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
En este apartado se muestra la metodología software seguida así como las tecnologías necesarias en la implementación del sistema y la herramienta de detección de entidades nombradas.
\section{Metodología}
Se ha optado por seguir la metodología Agile, tratando de seguir los principios del Manifiesto Agile \cite{Agile} sin hacer uso de un marco concreto de trabajo dado el tamaño del equipo.
...
\section{Tecnologías}
Para la implementación del sistema se hace uso de diferentes tecnologías y herramientas que se describen en esta sección. El lenguaje seleccionado para la implementación ha sido Python.\\

\subsection{Python}
Python es un lenguaje de programación interpretado muy usado en la actualidad, una de las cualidades que lo definen es su legibilidad. Puede ser usado con un paradigma imperativo, orientado a objetos y en menor medida funcional.

Dentro de este lenguaje se hace uso de diversas librerías y herramientas, algunas de ellas se describen en más detalle posteriormente, algunas de ellas son:

\begin{itemize}
\item re: Módulo de expresiones regulares nativo de python.
\item jupyter: Entorno web de desarrollo.
\item spaCy: Herramienta avanzada de PLN.
\item loggin: Módulo de generación y gestión de logs jerárquicos, es nativo de python.
\item shutil: Módulo que permite mover y copiar ficheros, es nativo de python.
\item os: Módulo que permite usar funcionalidad del sistema operativo, es nativo de python.
\item io: Módulo de gestión de ficheros que permite la lectura y escritura en diferentes formatos, es nativo de python.
\end{itemize}

\subsection{Jupyter}
La herramienta Jupyter es un entorno web open-source donde se puede ejecutar código y visualizar los resultados de una forma sencilla y estructurada. Esta herramienta es usada habitualmente por profesionales de ciencia de datos para limpiar, transformar y visualizar datos así como entrenar modelos entre otras tareas.\\

Hace uso de ficheros con extensión .ipynb a los que se llama notebook. Se usan diferentes notebooks para las distintas partes del sistema.

\subsection{SpaCy}
SpaCy es una herramienta open-source de PLN creada bajo la premisa de obtener software listo para producción, incluye funcionalidades como la tokenización, el reconocimiento de entidades nombradas (NER), el etiquetado gramatical (part-of-speech tagging) o la segmentación por oraciones entre otras.\\

El uso de esta herramienta en este caso ha sido para la tokenización y el reconocimiento de entidades nombradas.\\

Esta herramienta contiene diferentes modelos preentrenados para las diferentes tareas y para múltiples idiomas, pudiendo reentrenarse para ajustarse a los datos de la tarea en concreto.

\subsection{Docker}
Docker es una tecnología que permite la virtualización a nivel de sistema operativo en unos paquetes que se denominan contenedores, estos permiten empaquetar software y sus dependencias de forma sea reproducible, escalable e independiente de la plataforma que ejecute el contenedor.\\

Se hace uso de esta tecnología para la herramienta de etiquetado automático, permitiendo a cualquier usuario hacer uso de ella sin necesidad de instalar software a parte de docker.

\subsection{LaTeX}
LaTeX es un software de generación de documentos. A diferencia de otras herramientas de creación de documentos que siguen una filosofía de ver directamente el estilo del documento a medida que lo escribes, en LaTeX se escribe en texto plano con una sintaxis determinada de etiquetas que posteriormente se interpretarán para obtener un documento generado con el estilo correspondiente.\\

Este documento ha sido generado usando \LaTeX .
...
Existen dos casos de uso que aplican tanto en el sistema planteado para la tarea como para la herramienta de anotación automática, estos casos son el entrenado de modelos y la anotación de textos haciendo uso de esos modelos por parte del usuario. Existe también un caso de uso que aplica únicamente al sistema de la tarea, este caso es extraer la puntuación para una salida del sistema.
\subsection{Entrenamiento de un Modelo}
Para obtener un modelo entrenado el usuario debe proporcionar al sistema los textos anotados con los que se desea entrenar, en ese momento puede ejecutar en el sistema un entrenamiento que tendrá como resultado la obtención de un modelo listo para su uso. Una representación de este caso de uso puede verse en la Figura \ref{fig:caso-train}.
\begin{figure}[H]
\centering
\includegraphics[scale=0.65]{img/caso-train.png}\\
\caption{Entrenamiento de un modelo}
\label{fig:caso-train}
\end{figure}

\subsection{Anotación de Textos}
Partiendo de textos sin anotar y un modelo ya entrenado un usuario puede hacer uso del sistema para generar anotaciones sobre estos textos. La finalidad a la que está destinada la tarea MEDDOCAN es precisamente este caso de uso.\\

Un usuario debe en primer lugar proporcionar los textos sin anotar al sistema e indicar el modelo con el que se quiere anotar, posteriormente el usuario ejecutará una predicción del sistema que generará una copia de los textos originales junto con su anotación correspondiente como puede verse esquematizado en la Figura \ref{fig:caso-pred}.

\begin{figure}[H]
\centering
\includegraphics[scale=0.65]{img/caso-pred.png}\\
\caption{Anotación de textos a partir de un modelo}
\label{fig:caso-pred}
\end{figure}
\subsection{Obtener Puntuación}
En este caso el usuario ha obtenido el resultado del sistema en un directorio con los textos anotados, además de esto debe incorporar los mismos textos anotados de la forma correcta ya que será la referencia usada para la puntuación.\\

El usuario debe ejecutar la parte del sistema correspondiente a Score, esta a su vez hará uso de la herramienta proporcionada en la tarea MEDDOCAN \cite{Score} que dejará como resultado la puntuación, este caso queda recogido en la Figura \ref{fig:caso-score}.

\begin{figure}[H]
\centering
\includegraphics[scale=0.65]{img/caso-score.png}\\
\caption{Obtener puntuación para una salida}
\label{fig:caso-score}
\end{figure}
...
En primer lugar se lee el fichero de texto y se sustituyen todos los saltos de línea por espacios para evitar que se tomen las localizaciones de forma incorrecta. Tras ello se procede a leer el fichero de anotaciones, de este se extraen las etiquetas y localizaciones de las mismas en una lista donde cada elemento es una tupla con la estructura (inicio, final, etiqueta) y se corresponde a una entidad.\\

Cada caso de entrenamiento es una tupla de dos elementos, en primer lugar se encuentra el texto y en segundo lugar se encuentra un diccionario con una sola clave llamada \textit{entities} cuyo valor es una lista con todas las entidades anotadas para el texto. Un ejemplo de esta estructura para un fragmento de uno de los textos de ejemplo se muestra en la Figura \ref{fig:tag-example}.\\

\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{img/tag-example.png}\\
\caption{Ejemplo de entrada para un texto}
...
\end{lstlisting}
Las expresiones están formuladas de manera que eligiendo el primer grupo de la expresión se queda con el texto correspondiente a la etiqueta, de forma que para cada texto se extraen una lista de diccionarios que tienen como claves \textit{tag}, \textit{match}, \textit{ini} y \textit{fin} con todas las partes que encajen en una expresión, eligiendo el primer grupo como \textit{match}.
...
Finalmente, se cuenta el número de etiquetas en la anotación original para continuar con la numeración y se escriben las etiquetas de las expresiones regulares al final del fichero ann.
\subsection{Score}
...
\begin{itemize}
\item Verdadero Positivo (TP): Se elige la etiqueta y es la correcta.
\item Falso Positivo (FP): Se elige la etiqueta y no es correcta.
\item Verdadero Negativo (TN): No se elige la etiqueta y en efecto no es la correcta.
...
Esta métrica es una de las más simples y más usadas, se calcula tomando los verdaderos positivos y dividiendo entre la suma de los verdaderos positivos y los falsos positivos como se indica en la Ecuación \ref{eq:prec}. Esta métrica nos informa de cuantas veces se equivoca el modelo al predecir una etiqueta.\\
\begin{equation} \label{eq:prec}
\frac{TP}{TP+FP}
\end{equation}

Esta forma de puntuar tiene el inconveniente de que si las etiquetas tienen un volumen de aparición muy diferente es probable que aunque el valor sea alto, el modelo esté sesgado a predecir etiquetas de mayor volumen de aparición.

\subsubsection{Exhaustividad (Recall)}

En este caso se calcula tomando los verdaderos positivos y dividiendo entre la suma de verdaderos positivos y falsos negativos como muestra la Ecuación \ref{eq:recall}. Esta métrica se usa para conocer cuántos de los valores reales de una categoría puede identificar el modelo.
...
Esta métrica es la media armónica entre precisión y exhaustividad, su valor se encuentra entre 0 y 1, siendo 1 el mejor valor posible que indica una precisión y exhaustividad perfectas. La Ecuación \ref{eq:f1} muestra el cálculo de esta métrica.\\
\begin{equation} \label{eq:f1}
F1=2\cdot\frac{precision\cdot recall}{precision + recall}
\end{equation}

Esta será la métrica de referencia, que se usó en la tarea MEDDOCAN y que es habitual hacer uso de ella para determinar la calidad de los resultados en este tipo de problemas.
...
Las métricas para obtener las puntuaciones para la subtarea 1 son Leak, Precision, Recall y F1. La subtarea 2 tiene dos formas de puntuar, en la estricta tiene que coincidir la posición exacta de la entidad, mientras que en la forma combinada se da por valida si existen varias entidades que al combinarlas dan lugar a una válida. En este caso se presta mayor atención a la versión estricta y las métricas son Precision, Recall y F1. Siempre se toma como valor principal el F1.
\subsection{Conjunto de Validación}
En primer lugar se analizó la puntuación de los modelos en el conjunto de validación, el resultado de la primera tarea se puede ver en la Tabla \ref{tab:tabla-dev} donde se muestran los 15 con mejor puntuación F1. En la Tabla \ref{tab:tabla-dev-2} se muestran los resultados de la segunda tarea según el criterio estricto.\\
...
Con esta información se realiza un estudio de las posibles expresiones regulares para mejorar el sistema, se puede observar amplio margen de mejora en las entidades SEXO\_SUJETO\_ASISTENCIA y CALLE.\\
...
Según la clasificación general \cite{Guidelines} de la tarea, en la subtarea 1 se habría quedado justo por encima de la posición 11, mientas que en la subtarea 2 la posición sería la 13 de un total de 18 participantes.\\
...
En general los valores obtenidos son bastante buenos, dado que el modelo base que plantean obtiene unos resultados de $0.42668$ y $0.47181$ en la puntuación F1 para las subtareas 1 y 2 respectivamente, se supera notablemente esta línea base.


\cleardoublepage % empezamos en página impar
\chapter{Conclusiones}
El uso de la herramienta spaCy lleva a la conclusión de que partir de un modelo preentrenado es una buena idea ya que tiene información previa del lenguaje en el que están escritos los documentos por lo que con un reentrenamiento de su módulo de detección de entidades ofrece buenos resultados en un contexto que puede no haber visto nunca antes el modelo y generaliza estos resultados de forma correcta.\\
...
Dado que existen etiquetas que no se encuentran anotadas en los datos proporcionados y que tampoco se pueden detectar con expresiones regulares, el sistema nunca será capaz de detectarlas ni conoce de la existencia de esas etiquetas.
...
\item Prueba de otros modelos y sistemas para mejorar el actual.
